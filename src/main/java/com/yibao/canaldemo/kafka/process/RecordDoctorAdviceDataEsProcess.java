...
@Slf4j
@Service
public class RecordDoctorAdviceDataEsProcess implements KafkaMessageProcess {

    @Override
    public KafkaTopicEnum getTopic() {
        return KafkaTopicEnum.RECORD_DOCTOR_ADVICE;
    }

    @Override
    public void process(TableBean tableBean) {
        log.info("RecordDoctorAdviceDataEsProcess,tableBean:" + JSON.toJSONString(tableBean));
        switch (EventType.valueOfType(tableBean.getType()).getCode()) {
            case CanalEntry.EventType.INSERT_VALUE:
                ...
                log.info("recordI=" + insertRowData.get("record_id") + ",patientId=" + insertRowData.get("patient_id"));
            ...
            case CanalEntry.EventType.UPDATE_VALUE:
                ...
                log.info("recordI=" + updateRowData.get("record_id") + ",patientId=" + updateRowData.get("patient_id"));
            ...
            case CanalEntry.EventType.DELETE_VALUE:
                ...
                log.info("recordI=" + deleteRowData.get("record_id") + ",patientId=" + deleteRowData.get("patient_id"));
